...
public sealed class ResetPasswordCommandHandler : IRequestHandler<ResetPasswordCommand, ErrorOr<AuthResult>>
{
    ...
    public ResetPasswordCommandHandler(IReadRepository<User, UserId> usersReadRepo, IWriteUserRepositry writeUserRepositry, ILogger<ResetPasswordCommandHandler> logger, IJwtTokenGenerator jwtTokenGenerator, ICurrentUser currentUser, IWriteUnitOfWork writeUnitOfWork)
    {
    ...
    public async Task<ErrorOr<AuthResult>> Handle(ResetPasswordCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            user.AddUserCredential(UserCredentials.Create(user.Id, command.newPassword), _currentUser?.UserId?.ToString() ?? "unkown");
